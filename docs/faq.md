...
> 2. [Dataset and Configuration](#dataset-and-configuration)
> 3. [Training (Toolbox and CLI)](#training-toolbox-and-cli)
> 4. [Toolbox Overview](#toolbox-overview)
> 5. [Testing and Reviewing Results](#testing-and-reviewing-results)
> 6. [ONNX Export and Deployment](#onnx-export-and-deployment)
> 7. [Scheduling, Losses and Internals](#scheduling-losses-and-internals)
...
Run the test suite with pytest. See [here](testing_your_installation.md) for more information.
...
```
> [!NOTE]
> The `test` directory is optional. The dataset images in this directory can be used in a separate post-train model validation. See [here](getting_started/toolbox_testing.md).

### 2) How do I set/override config values?
**Toolbox:** The Toolbox generates training configs dynamically. When using it, all config values can be set via the UI. See [here](getting_started/toolbox_training.md).

**CLI:** The default config file used by the training scripts can be found at [`nectargan/config/default.json`](https://github.com/ZacharyBork/NectarGAN/blob/main/nectargan/config/default.json). You can open this file in any text editor to override values. See [here](api/config.md) for more information.

**Custom Training Script:** There are a number of ways to manage configuration options when writing a custom training script. See [here](api/config.md) for more information. 

### 3) Can I train on grayscale or multi-channel images?
This can be controlled by setting the value of `in_channels` on the [UNet](api/models/unet.md) and [PatchGAN](api/models/patchgan.md) models. 1 for grayscale, 3 for RGB. I haven't tested multi-channel inputs much; you may encounter problems. I will check on this and fix it if necessary at some point in the future. ONNX model conversion currently only supports 3 channels. See [here](toolbox/utilities.md) and [here](api/onnx_tools.md) for more information on ONNX conversion.

### 4) How do I select the mapping direction (A->B vs B->A)?
By setting the value of `config.dataloader.direction` in your config file. Valid values are `AtoB` and `BtoA`. See [here](api/dataset.md) for more information regarding dataset loading.

## Training (Toolbox and CLI)

### 1) How do I start Pix2pix training from the CLI?
See [here](getting_started/cli.md).
...
See the [documentation on loss functions](api/losses/loss_functions.md) and the [Pix2pix objective function](https://github.com/ZacharyBork/NectarGAN/blob/main/nectargan/losses/pix2pix_objective.py) for more information on the behaviour of these loss subspecs.
...
**Custom Training Script:** See the [LossManager](api/losses/lossmanager.md) documentation for information on setting loss weights and applying weight schedules.
...
**CLI:** Yes. Just use the `-f` `--config_file` flag and pass it the path the the config file for the epoch you would like to resume training from. See the [CLI documentation](getting_started/cli.md) for more information.

**Custom Training Script:** See [here](api/trainers/building_a_trainer.md).

### 5) How are learning rates scheduled?
**Toolbox:** On the Training panel, you can set a learning rate schedule with the `Epochs`, `Epochs Decay`, and `Initial` and `Target` learning rate. See [here](api/scheduling/schedule_functions.md) for more info. 
...
**Custom Training Script:** See [here](api/scheduling.md) for more information on scheduling.
...
Experiments will be exported to the `Output Root`, in directories named `Experiment Name`. These directories are versioned automatically (see [here](toolbox/experiment.md)). **`Output Root` and `Experiment Name` are set in different places depending upon how training is initiated:**
...
**Toolbox:** Using the Testing panel. See [here](getting_started/toolbox_testing.md)

**CLI:** See [here](getting_started/cli.md) for information on model testing via CLI.

**Custom Testing Script:** See the [Tester class documentation](api/testers.md).

### 2) How do I review my experiments?
The Toolbox Review panel offers an easy way to review the results of your model test. See [here](getting_started/toolbox_review.md) and [here](toolbox/review.md) for more information.
...
**CLI:** Using the `--log_losses` flag. See [here](getting_started/cli.md).

**Custom Training Script:** See the [LossManager documentation](api/losses/lossmanager.md).

## ONNX Export and Deployment

### 1) How do I export to ONNX?
**Toolbox:** On the Utilities panel, you will find a set of tools that allow you to convert your models to run on the ONNX runtime, and to test your converted models. See [here](toolbox/utilities.md) for more information. 
...
See the [Loss Manager documentation](api/losses/lossmanager.md) for more information.
...
See [here](api/scheduling.md) to get started with scheduling in NectarGAN.

## Troubleshooting

### 1) Training is slow in the Toolbox. Any tips?
Increase Training Update Rate (see [here](toolbox/settings.md)), and avoid very low dump frequencies for loss logs, as these can potentially cause lag spikes.
...
If the `-f` flag isn't used, the training/testing scripts will instead use the default config file located at [`nectargan/config/default.json`](https://github.com/ZacharyBork/NectarGAN/blob/main/nectargan/config/default.json).
