...
> [!IMPORTANT]
> This section will largely build upon topics addressed in the [documentation](../trainers/trainer.md) for the base `Trainer` class. It is advisable to read that before document before beginning this one.

## The `Pix2pixTrainer` class
The [`Pix2pixTrainer`](https://github.com/ZacharyBork/NectarGAN/blob/main/nectargan/trainers/pix2pix_trainer.py) serves as an example implementation of a `Trainer` subclass, a Pix2pix-style[^1] GAN model in this case.
...
| :---: | --- |
[`_init_lr_scheduling`](https://github.com/ZacharyBork/NectarGAN/blob/main/nectargan/trainers/pix2pix_trainer.py#L51) | Checks the value of `separate_lr_schedules` in the current config. If it is `True`, it does nothing. If it is `False`, however, it will override the discriminator's learning rate with the value of the generator's learning rate, allowing learning rates for the discriminator to be accessed the same way, regardless of whether it is using a separate schedule.
[`_init_generator`](https://github.com/ZacharyBork/NectarGAN/blob/main/nectargan/trainers/pix2pix_trainer.py#L57) | Initializes a [`UnetGenerator`](../models/unet.md) network, and a `torch.nn.optim.Adam` optimizer for the network.
[`_init_disriminator`](https://github.com/ZacharyBork/NectarGAN/blob/main/nectargan/trainers/pix2pix_trainer.py#L84) | Initializes a PatchGAN-style [`Discriminator`](../models/patchgan.md) network, and a `torch.nn.optim.Adam` optimizer for the network.
[`_init_dataloaders`](https://github.com/ZacharyBork/NectarGAN/blob/main/nectargan/trainers/pix2pix_trainer.py#L109) | Initializes one `torch.nn.data.DataLoader` each for the `train` and `val` dataset.
[`_init_gradscalers`](https://github.com/ZacharyBork/NectarGAN/blob/main/nectargan/trainers/pix2pix_trainer.py#L114) | Initializes a gradient scaler for both G and D based on the device in the current config.

**Once those are completed, we initialize our losses with the `Trainer`'s `LossManager`.** This is done here by running the function [`Pix2pixTrainer._init_losses()`](https://github.com/ZacharyBork/NectarGAN/blob/main/nectargan/trainers/pix2pix_trainer.py#L122). You are encouraged to read the docstring for this function as it is extremely thorough, but in short, it takes the `loss_subspec` argument which was passed to the `Pix2pixTrainer` at init, validates it against a list of supported subspecs, and then passes that along with the input `config` and the a reference to the NectarGAN (see [here](../losses/loss_spec.md) and [here](https://github.com/ZacharyBork/NectarGAN/blob/main/nectargan/losses/pix2pix_objective.py)) to the `init_from_spec()` function of the `Trainer`'s internal [`LossManager`](../losses/lossmanager.md). This registers all the losses required loss functions for training.

**Finally, we do a quick check of the current config to see if we are continuing a train on a pre-existing model.** If we are, we run the [`load_checkpoint()`](https://github.com/ZacharyBork/NectarGAN/blob/main/nectargan/trainers/trainer.py#L159) function from the parent `Trainer` class to load the weights for both the generator and the disriminator.
...
> ###### From [`docs/api/trainers/trainer.md`](../trainers/trainer.md)
> | Function | Description |
> | :---: | --- |
> [`on_epoch_start`](https://github.com/ZacharyBork/NectarGAN/blob/main/nectargan/trainers/trainer.py#L262) | Run at the very beginning of an epoch in [`Trainer.train_paired()`](https://github.com/ZacharyBork/NectarGAN/blob/main/nectargan/trainers/trainer.py#L353), before the training loop begins.
> [`train_step`](https://github.com/ZacharyBork/NectarGAN/blob/main/nectargan/trainers/trainer.py#L282) | Run once per batch in [`Trainer.train_paired()`](https://github.com/ZacharyBork/NectarGAN/blob/main/nectargan/trainers/trainer.py#L353) -> [`Trainer._train_paired_core()`](https://github.com/ZacharyBork/NectarGAN/blob/main/nectargan/trainers/trainer.py#L337).
> [`on_epoch_end`](https://github.com/ZacharyBork/NectarGAN/blob/main/nectargan/trainers/trainer.py#L313) | Run at the very end of an epoch in [`Trainer.train_paired()`](https://github.com/ZacharyBork/NectarGAN/blob/main/nectargan/trainers/trainer.py#L353), after all the batches for the epoch have been completed.
...
3. `train_step` is different. It does still accept the `kwargs` dict like the other two, but it also has 3 required arguments, all of which are passed to it during the training loop in [`Trainer.train_paired()`](https://github.com/ZacharyBork/NectarGAN/blob/main/nectargan/trainers/trainer.py#L353) -> [`Trainer._train_paired_core()`](https://github.com/ZacharyBork/NectarGAN/blob/main/nectargan/trainers/trainer.py#L337). These are:
...
---
**Now let's have a look at the function that executes all three of the callbacks, [`Trainer.train_paired()`](https://github.com/ZacharyBork/NectarGAN/blob/main/nectargan/trainers/trainer.py#L353), to better contextualize how they function.** 
...
> ##### From [`/nectargan/start/training/paired.py`](https://github.com/ZacharyBork/NectarGAN/blob/main/nectargan/start/training/paired.py)
> ```python
...
> ##### From [`nectargan.trainers.trainer.Trainer.train_paired`](https://github.com/ZacharyBork/NectarGAN/blob/main/nectargan/trainers/trainer.py#L)
> ```python
> [...]
>
...
> ##### From [`nectargan.trainers.pix2pix_trainer.Pix2pixTrainer`](https://github.com/ZacharyBork/NectarGAN/blob/main/nectargan/trainers/pix2pix_trainer.py#L428)
> ```python
...
> ```
Looking at our callback method, we can see that it treats the incoming `kwargs` argument as a dict. It first checks the items of the dict to see if `print_train_start` is present and, if it is, it then checks to see if the value is `True` or `False`. It it is `False`, nothing happens. If it is `True`, though, it will print a short string showing the index of the epoch which is about to begin, and then calls its `LossManager`'s [`print_weights`](https://github.com/ZacharyBork/NectarGAN/blob/main/nectargan/losses/loss_manager.py#L342) to print all of the loss weights which will be used in the upcoming epoch.
...
> ##### From [`nectargan.trainers.pix2pix_trainer.Pix2pixTrainer`](https://github.com/ZacharyBork/NectarGAN/blob/main/nectargan/trainers/pix2pix_trainer.py#L472)
> ```python
> def train_step(
>         self, 
>         x: torch.Tensor, 
...
---
### Recap
**Alright, so we've seen our three different callback functions, and we've had a quick look at the [function that invokes them](https://github.com/ZacharyBork/NectarGAN/blob/main/nectargan/trainers/trainer.py#L353). Now let's briefly recap each, and have a quick look at what is does, how and where exactly each is used in the base `Trainer`, and how the `Pix2pixTrainer` handles overriding each method, so that we can better understand how they can be used to quickly prototype training loops for our other models.**
### `on_epoch_start`
**Description:** This callback is run once at the [very beginning of the epoch](https://github.com/ZacharyBork/NectarGAN/blob/main/nectargan/trainers/trainer.py#L408), before the actual training loop is started.
...
### `train_step`
**Description:** Run once per batch during each epoch, not directly by the `train_paired` function, but instead via the private [`_train_paired_core`](https://github.com/ZacharyBork/NectarGAN/blob/main/nectargan/trainers/trainer.py#L337)

**Use:** Forward and backward train step.
### `on_epoch_end`
**Description:** This callback is run once at the [very end of the epoch](https://github.com/ZacharyBork/NectarGAN/blob/main/nectargan/trainers/trainer.py#L408), after all of the batches for that epoch have been completed.
