...
> [*`NectarGAN API - Home`*](/docs/api.md) [*`Getting Started (Toolbox | Testing)`*](/docs/getting_started/toolbox_testing.md)
...
## The `Tester` class
> **Reference:** [`nectargan.testers.tester`](/nectargan/testers/tester.py)
...
The first thing we notice is that it inherits from the base [`Trainer`](/docs/api/trainers/trainer.md) class. The `Tester` uses its parent class to:
...
| :---: | --- |
`config` | This can be any number of things representing a config (see [here](/docs/api/config.md)), or `None` (default). This config is primarily used to define generator architecture settings, although it may also be used for a few other things depending on the other arguments which are passed to it.<br><br> **If the generator you are loading for testing uses different architecture settings than the default config, you will need to pass a config (like the default train config, exported automatically for each train). Otherwise you can use the default `None`**
...
2. Force the `continue_train` boolean value in the config to `True`. We do this so that we can call the parent class' [`build_output_directory`](/nectargan/trainers/trainer.py) method to initialize the current `self.experiment_dir` (*a member variable from the parent `Trainer` class which defines the current experiement directory*) path without it raising an exception. See [here](/nectargan/testers/tester.py#L79) for more info. 
...
4. Then check if the input `experiment_directory` is `None`:
    - **If it is:** We run [`Tester._build_output_directory()`](/nectargan/testers/tester.py#L79). This calls `Trainer.build_output_directory()`, as discussed above, then calls [`Tester._init_test_output_root()`](/nectargan/testers/tester.py#L65). All this function does is perform a quick check of the current `experiment_directory` to see if it contains a subdirectory called `test`. If not, it tries to make one.
    - **If it isn't:** We just set `self.experiment_dir` to the input `experiement_directory`, then call [`Tester._init_test_output_root()`](/nectargan/testers/tester.py#L65).
...
### Testing
> **Reference:** [`Tester.run_test`](/nectargan/testers/tester.py#L258)
...
2. Evalute the generator's output using a few loss function (currently [`L1`](https://docs.pytorch.org/docs/stable/generated/torch.nn.L1Loss.html), [`Sobel`](/docs/api/losses/loss_functions.md), [`Laplacian`](/docs/api/losses/loss_functions.md))
...
NectarGAN provides a prebuild script for interacting with the `Tester`, allowing you to run model tests from the command line. For more information, please see [here](/docs/getting_started/cli.md).
...
1. [**`TesterWorker`**](/nectargan/toolbox/workers/testerworker.py)

    A worker class which performs the actual testing inside of a `QThread`. `TesterWorker` is a child of `Tester`, and uses basically all of its functionality wholesale. The only difference is that it implements its own version of `Tester.run_test()`, [`TesterWorker.run()`](/nectargan/toolbox/workers/testerworker.py#L29).
...
2. [**`TesterHelper`**](/nectargan/toolbox/helpers/tester_helper.py)
    ...
    - [`_parse_test_log`](/nectargan/toolbox/helpers/tester_helper.py#L223)
    ...
    - [`_sort_results`](/nectargan/toolbox/helpers/tester_helper.py#L102)
    ...
    - [`load_test_results`](/nectargan/toolbox/helpers/tester_helper.py#L233)
