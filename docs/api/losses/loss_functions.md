...
On the right is the result of an L1 loss function applied to the two Sobel maps. The `Mean` at the bottom is the final loss value (at least in the default implementation which uses a mean reduction on the returned L1 loss result).
## Laplacian Loss
Reference: [`nectargan.losses.losses.Laplacian`](/nectargan/losses/losses.py#L53)

Laplacian loss is very similar to Sobel loss, except instead of applying a Sobel filter to the ground truth and generator fake, it instead applies a Laplacian filter. This can encourage the generator to preserve more small-scale textural detail from the ground truth image. For some tasks, it can be used alongside Sobel loss as a very effective replacement for L1 loss. It can also help dissuade the generator from averaging inputs which can sometimes result in blurry outputs.
...
[^1]: Disclaimer: While the `y_fake` in these examples is a real example output from the NectarGAN Pix2pix implementation, the example Sobel and Laplacian images and the loss maps were instead generated post-train via [Copernicus](https://www.sidefx.com/docs/houdini/copernicus/index.html) in Houdini. Please note, however, that **it is possible to extract loss maps as tensors during training natively.** For more information, please see [here](/docs/api/losses/lossmanager.md) and [here](/nectargan/losses/loss_manager.py#L236).
