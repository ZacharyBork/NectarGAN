...
from nectargan.models.diffusion.model import DiffusionModel
...
class DiffusionTrainer(Trainer):
    def __init__(
        ...
        self.diffusion_model = DiffusionModel(
            self.train_timesteps, self.device)
    ...
    def update_display(
        ...
        if self.config.visualizer.visdom.enable:
            self.vis.update_images( # Update x, y_fake, y image grid
                x=x, y=y, z=z, title='Input | Output | Predicted', 
            ...
            self.vis.update_loss_graphs(graph_step, losses_G, { 'test': 0.0 })

    def export_examples(self, x: torch.Tensor, idx: int) -> None:
        with torch.amp.autocast('cuda'):
            with torch.no_grad():
                output = self.diffusion_model.sample(x.shape)
    ...
    def train_step(
        ...
        with torch.amp.autocast('cuda'):
            ...
            predicted_noise = self.diffusion_model.autoencoder(x_t, timesteps)
            loss_G_MSE = self.loss_manager.compute_loss_xy(
                'G_MSE', predicted_noise, noise, self.current_epoch)
        self.backward(loss_G_MSE)

        if idx % self.config.visualizer.visdom.update_frequency == 0:
            self.update_display(x, x_t, predicted_noise, idx)
        if idx % 2000 == 0: self.export_examples(x, idx)
