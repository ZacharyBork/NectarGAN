...
CONFIG_FILE="/app/mount/train_config.json"
chmod +x $CONFIG_FILE

# Update IO pathing in the train config file
jq '.config.common.output_directory = "/app/mount/output" | .config.dataloader.dataroot = "/app/mount/input"' \
...
jq '.config.visualizer.visdom.enable = true | .config.visualizer.visdom.port = 8000' \
    "$CONFIG_FILE" > config.tmp && mv config.tmp "$CONFIG_FILE"
